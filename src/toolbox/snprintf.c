...
local_function void t__snprintf_add_string(t__SnprintfHead *head, char *s)
{
    while (*s) {
...
local_function void t__snprintf_add_uint(t__SnprintfHead *head, unsigned int value)
{
    if (value == 0) {
    ...
    do {
        int digit = (value - acc) / t_powi(10, ndigits - 1);
        acc += digit * t_powi(10, ndigits - 1);
        ndigits--;
        char c = (char)(digit + '0');
...
local_function void t__snprintf_add_int(t__SnprintfHead *head, int value)
{
    if (value >= 0) {
...
local_function void t__snprintf_add_float(t__SnprintfHead *head, float value, int digits)
{
    ...
    int32_t fracpart = (int32_t)((value - intpart) * t_powi(10, digits));
...
local_function void t__snprintf_parse_flags(t__SnprintfFlags *flags, char **head)
{
...
local_function int t__snprintf_parse_uint(char **head, int *result)
{
    if (!t_isnum(**head))
        return 0;
...
local_function void t__snprintf_parse_width(t__SnprintfWidth *width, char **head)
{
...
local_function void t__snprintf_parse_precision(int *precision, char **head)
{
    if (**head != '.') {
        *precision = 6;
        return;
