...
static ErrorCode platform_win32_init_wgl_extensions() {
    ...
    HWND wh = CreateWindow(wc.lpszClassName,
                           "PlatformTempoWindow",
                           0,
                           CW_USEDEFAULT,
                           CW_USEDEFAULT,
...
static ErrorCode platform_win32_init_gl(Platform *p) {
    ...
    // Load gl function pointers (since we can only load
    // them after the context is created...)
