...
#define maybe_some(type, v)                                \
    ((Maybe(type)){.is_something = true, .value = v})
...
#define maybe_unwrap(m)                                    \
    (maybe_is_nothing(m) ? panic() : 0, m.value)
#define maybe_ifsome(m) if (maybe_is_something(m))
#define maybe_set_some(m, v)                               \
    (m.is_something = true, m.value = v)
#define maybe_set_nothing(m) m.is_something = false;
#define maybe_map(type, m, f)                              \
    (maybe_is_something(m) ? maybe_some(type, f(m.value))  \
                           : maybe_nothing(type))
#define maybe_lift(type, m1, m2, f)                        \
    (maybe_is_something(m1) && maybe_is_something(m2)      \
         ? maybe_some(type, f(m1.value, m2.value))         \
         : maybe_nothing(type))
