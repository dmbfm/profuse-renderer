...
boolean heap_wasm_memory_grow(HeapWasmMemory *m,
                              usize           amount) {
    usize pagesneeded =
        math_div_ceil_u32(amount, m->bytes_per_page);
...
Result(uptr)
    heap_wasm_memory_bump(HeapWasmMemory *m, usize amount) {
...
Result(uptr) heap_wasm_memory_bump_align(HeapWasmMemory *m,
                                         usize amount,
                                         usize alignment) {
    if (!m->initialized) heap_wasm_memory_init(m);

    uptr current = m->start + m->offset;
    Result(uptr) raligned =
        mem_align_pointer_forward(current, alignment);
    ...
    Result(uptr) rptr =
        heap_wasm_memory_bump(m, amount + delta);
...
local ErrorCode wasm_free_list_allocator_init(
    HeapWasmFreeListAllocatorState *allocator,
    usize                           default_alignment) {
    ...
    allocator->default_alignment = default_alignment == 0
                                       ? HEAP_WASM_MAX_ALIGN
                                       : default_alignment;
    ...
    // user pointer
    Result(uptr) r_first = mem_align_pointer_forward(
        allocator->heap.start + sizeof(BlockHeader),
    ...
    // we're out of memory
    if ((allocator->heap.end - sizeof(BlockFooter)) <=
        first) {
    ...
    usize block_size =
        (allocator->heap.end - sizeof(BlockFooter)) - first;

    assert(block_size);

    // Define the block's header and footer
    BlockHeader header = {.flag = BLOCK_FREE,
                          .size = block_size,
                          .next = &allocator->head,
                          .prev = &allocator->head};
    BlockFooter footer = {.flag = BLOCK_FREE,
                          .size = block_size};

    // Calculate where the header and footer should be
    // stored at
    BlockHeader *header_ptr =
        (BlockHeader *)(first - sizeof(BlockHeader));
    BlockFooter *footer_ptr =
        (BlockFooter *)(allocator->heap.end -
                        sizeof(BlockFooter));
...
local void
wasm_free_list_allocator_remove_block(BlockHeader *h) {
    h->next->prev = h->prev;
    h->prev->next = h->next;
}

local void wasm_free_list_allocator_prepend_block(
    HeapWasmFreeListAllocatorState *allocator,
    BlockHeader *                   h) {
...
local void wasm_free_list_print(
    HeapWasmFreeListAllocatorState *allocator,
    void (*printfn)(const char *)) {
    if (!allocator->initialized)
        wasm_free_list_allocator_init(allocator,
                                      HEAP_WASM_MAX_ALIGN);
    ...
    while ((current = current->next) != &allocator->head) {
        format(b,
               ...
               current->size,
               (uptr)current,
               (uptr)current + sizeof(BlockHeader) +
                   sizeof(BlockFooter) + current->size);
        printfn(b);
        i++;
    }
...
HeapWasmFreeListRegionInfo
wasm_free_list_region_info(uptr region) {
    HeapWasmFreeListRegionInfo info = {0};

    info.header =
        (BlockHeader *)(region - sizeof(BlockHeader));
    info.footer =
        (BlockFooter *)(region + info.header->size);
    info.header_start = (uptr)info.header;
    info.footer_end =
        (uptr)info.footer + sizeof(BlockFooter);
...
void heap_wasm_free_list_print_region_info(
    void (*printfn)(const char *),
    uptr region) {
    HeapWasmFreeListRegionInfo info =
        wasm_free_list_region_info(region);
    ...
    format(
        ...
        256,
        "\tFlag: %s",
        block_state_string((BlockState)info.header->flag));
    printfn(buf);
    format(buf,
           256,
           "\tRange: [%d, %d]",
...
local Result(uptr) wasm_free_list_allocator_alloc(
    HeapWasmFreeListAllocatorState *allocator,
    usize                           amount) {
    ...
    usize amount_needed_for_new_block =
        result_unwrap(mem_align_pointer_forward(
            (uptr)(amount + sizeof(BlockHeader) +
                   sizeof(BlockFooter)),
            allocator->default_alignment));
    ...
    while ((current = current->next) != &allocator->head) {
        ...
        if (current->size >= amount) {
            ...
            if (current->size >=
                ...
                usize next_block_size =
                    initial_size - sizeof(BlockFooter) -
                    sizeof(BlockHeader) - allocated_amount;

                // Make sure it is larger than the min block
                // size
                assert(next_block_size >=
                       allocator->min_block_size);
                ...
                // list
                wasm_free_list_allocator_remove_block(
                    current);

                // Set the footer of the current block
                uptr current_footer = (uptr)current +
                                      sizeof(BlockHeader) +
                                      current->size;
                ((BlockFooter *)current_footer)->flag =
                    BLOCK_RESERVED;
                ((BlockFooter *)current_footer)->size =
                    current->size;

                // Set the header of the created block
                uptr next_header = (uptr)current_footer +
                                   sizeof(BlockFooter);
                ((BlockHeader *)next_header)->flag =
                    BLOCK_FREE;
                ((BlockHeader *)next_header)->size =
                    next_block_size;

                assert(next_header ==
                       user_pointer + current->size +
                           sizeof(BlockFooter));

                // Set the footer of the created block
                uptr next_footer =
                    next_header + sizeof(BlockHeader) +
                    next_block_size;    //(uptr)current +
                                        //initial_size;
                ((BlockFooter *)next_footer)->flag =
                    BLOCK_FREE;
                ((BlockFooter *)next_footer)->size =
                    next_block_size;

                // Add the created block to the free list
                wasm_free_list_allocator_prepend_block(
                    allocator,
                ...
                BlockFooter *footer =
                    (BlockFooter *)current +
                    sizeof(BlockHeader) + current->size;
...
static void wasm_free_list_allocator_free(
    HeapWasmFreeListAllocatorState *allocator,
    uptr                            region) {
    HeapWasmMemory *memory = &allocator->heap;
    BlockHeader *   header =
        (BlockHeader *)((uptr)region - sizeof(BlockHeader));
    BlockFooter *left_footer =
        (BlockFooter *)((uptr)header - sizeof(BlockFooter));

    BlockFooter *footer =
        (BlockFooter *)((uptr)region + header->size);
    BlockHeader *right_header =
        (BlockHeader *)((uptr)footer + sizeof(BlockFooter));
    ...
    wasm_free_list_allocator_prepend_block(allocator,
                                           header);
    ...
    // Merge left
    if (((uptr)left_footer >
         (memory->start + sizeof(BlockHeader))) &&
        (left_footer->flag == BLOCK_FREE)) {

        BlockHeader *left_header =
            (BlockHeader *)((uptr)left_footer -
                            left_footer->size -
                            sizeof(BlockHeader));

        assert(left_header->flag == BLOCK_FREE);

        left_header->size = (uptr)footer -
                            (uptr)left_header -
                            sizeof(BlockHeader);
    ...
    // Merge right
    if (((uptr)right_header < memory->end) &&
        right_header->flag == BLOCK_FREE) {

        wasm_free_list_allocator_remove_block(right_header);

        BlockFooter *right_footer =
            (BlockFooter *)((uptr)right_header +
                            right_header->size +
                            sizeof(BlockHeader));

        header->size = ((uptr)right_footer - (uptr)header) -
                       sizeof(BlockHeader);
...
static HeapWasmFreeListAllocatorState
    wasm_free_list_allocator_state;

static Result(uptr)
    wasm_free_list_alloc(Allocator *allocator,
                         usize      amount) {
    return wasm_free_list_allocator_alloc(
        &wasm_free_list_allocator_state,
        amount);
}

static void wasm_free_list_free(Allocator *allocator,
                                uptr       region) {
    wasm_free_list_allocator_free(
        &wasm_free_list_allocator_state,
        region);
}

Allocator heap_wasm_free_list_allocator = {
    .alloc = wasm_free_list_alloc,
    .free  = wasm_free_list_free};

void heap_wasm_free_list_print_blocks(
    void (*printfn)(const char *)) {
    wasm_free_list_print(&wasm_free_list_allocator_state,
                         printfn);
}
...
void wasm_free_list_print_all(
    ...
    if (!wasm_free_list_allocator_state.initialized)
        wasm_free_list_allocator_init(
            &wasm_free_list_allocator_state,
            HEAP_WASM_MAX_ALIGN);

    BlockHeader *header =
        wasm_free_list_allocator_state.first;

    printfn(
        "=========== Wasm Allocator Blocks ===========");

    while ((uptr)header <
           wasm_free_list_allocator_state.heap.end) {
        heap_wasm_free_list_print_region_info(
            printfn,
            (uptr)header + sizeof(BlockHeader));
        header =
            (BlockHeader *)((uptr)header + header->size +
                            sizeof(BlockFooter) +
                            sizeof(BlockHeader));
    }
...
u32 wasm_memory_grow(u32 pages) {
    if ((__mock_memory.num_pages + pages) >
        __mock_memory.max_pages)
...
void __mock_memory_init() {
    __mock_memory.base =
        (uptr)malloc(10 * HEAP_WASM_PAGE_SIZE);
    expect(__mock_memory.base);
    expect(mem_is_aligned(__mock_memory.base,
                          ALLOCATOR_MAX_ALIGNMENT));
...
void heap_wasm_memory_init_custom_base(HeapWasmMemory *m,
                                       uptr base) {
    m->start          = base;
...
test(heap_wasm_memory_bump) {
    ...
    HeapWasmMemory  mm = {0};
    HeapWasmMemory *m  = &mm;
    heap_wasm_memory_init_custom_base(m,
                                      __mock_memory.base);
...
test(heap_wasm_memory_bump_align) {
    ...
    Result(uptr) rx = heap_wasm_memory_bump_align(
        &m,
        10,
        ALLOCATOR_MAX_ALIGNMENT);
    expect(result_is_ok(rx));
    expect(
        mem_is_aligned(rx.value, ALLOCATOR_MAX_ALIGNMENT));
...
test(wasm_free_list_allocator) {
    ...
    heap_wasm_memory_init_custom_base(&a.heap,
                                      __mock_memory.base);
    wasm_free_list_allocator_init(&a, 8);

    Result(uptr) rx =
        wasm_free_list_allocator_alloc(&a, 100);

    expect(result_is_ok(rx));
    expect(rx.value ==
           result_unwrap(mem_align_pointer_forward(
               a.heap.start + sizeof(BlockHeader),
               HEAP_WASM_MAX_ALIGN)));
