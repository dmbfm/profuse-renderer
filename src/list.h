...
// NOTE: Will using the header like we do in here cause
// alignment issues?
...
// Create a new list
#define list_new(type, a)                                  \
    (type *)__list_init(a, 0, sizeof(type))

// Create a list with given capacity
#define list_new_cap(type, a, cap)                         \
    (type *)__list_init(a, cap, sizeof(type))

// Returns a pointer to the list header
#define __list_hdr(l)                                      \
    ((ListHdr *)((uptr)(l) - sizeof(ListHdr)))
...
#define __list_needs_grow(l, n)                            \
    (list_len(l) + (n) >= list_cap(l))

// Grow if it needs to, else do nothing
#define __list_maybe_grow(l, n)                            \
    (__list_needs_grow(l, n) ? __list_grow(l, n) : 0)

// Actually grow the list
#define __list_grow(l, n)                                  \
    (*((uptr *)&(l)) =                                     \
         __list_growf((uptr)(l), (n), sizeof(*(l))))

// Push an element to the list
#define list_push(l, v)                                    \
    (__list_maybe_grow(l, 1), (l)[list_len(l)++] = (v))

// Free the list
#define list_free(l)                                       \
    (__list_allocator(l)->free(__list_allocator(l),        \
                               (uptr)l - sizeof(ListHdr)), \
     (l) = 0)
...
#define list_for(name, l)                                  \
    for (usize name = 0; name < list_len(l); name++)

#define list_as_slice(type, l)                             \
    slice_from_array(type, (l), list_len(l))

static inline uptr __list_growf(uptr  list,
                                usize increment,
                                usize element_size) {
    if (!list) panic();
    ...
    usize new_cap = c_max(2 * list_cap(list),
                          list_len(list) + increment);
    usize new_size =
        new_cap * element_size + 2 * sizeof(ListHdr);
...
static inline uptr
__list_init(Allocator *a, usize cap, usize element_size) {
    if (cap == 0) cap = 10;
